...
public class TeacherController {
  ...
  @PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_GENERAL_MANAGER') or "
      + "(hasRole('ROLE_TEACHER') and authentication.principal.id == #id)")
    ...
    if (updateTeacherDto.getSubjectId() != null) {
      ...
      if (subject == null) {
        throw new NotFoundException("Subject", "id",
            updateTeacherDto.getSubjectId());
      }
    }

    Teacher existingTeacher = teacherService.findById(id);
    Optional.ofNullable(updateTeacherDto.getPosition())
        .map(Position::fromString)
        .ifPresent(existingTeacher::setPosition);
    Optional.ofNullable(updateTeacherDto.getSubjectId())
        .ifPresent(existingTeacher::setSubjectId);
